...
def find_students(term):
    ...
    for student in student_db:
        if term.lower() in student.name.lower():
            results_db.append(student)
    if not results_db:
        print("No match found")
    else:
        for result in results_db:
            print(f"Student ID: {result.id}, Teacher Name: {result.name}, Enrolled in: {result.enrolled_in}")
...
def find_teachers(term):
    ...
    for teacher in teacher_db:
        if term.lower() in teacher.name.lower():
            results_db.append(teacher)
    if not results_db:
        print("No match found")
    else:
        for result in results_db:
            print(f"Teacher ID: {result.id}, Teacher Name: {result.name}, Speciality: {result.speciality}")
